...
#if DEBUG
    #define FAIL_ON_ERROR_OBJECT_CATEGORY(error) {if (error) {NSLog(@"ERROR: %s, %@", __FUNCTION__, error); __builtin_trap();}}
#else
    #define FAIL_ON_ERROR_OBJECT_CATEGORY(error) {if (error) {NSLog(@"ERROR: %s, %@", __FUNCTION__, error); abort();}}
#endif
...
+ (NSUInteger)allEntriesCountInContext:(NSManagedObjectContext *)context {
    ...
    FAIL_ON_ERROR_OBJECT_CATEGORY(error);
